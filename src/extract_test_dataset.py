...
def drop_invalid_appointment_pairs(df):
    """
    Drops records where the next scheduled appointment is before the encounter date.
    These are invalid from a timeline perspective.
    """
    logging.info("Dropping invalid rows: Next appointment earlier than encounter...")

    # Ensure datetime conversion (robust fix)
    df['Next_clinical_appointment'] = pd.to_datetime(df['Next_clinical_appointment'], errors='coerce')
    df['Encounter_Date'] = pd.to_datetime(df['Encounter_Date'], errors='coerce')

    before = len(df)
    df = df[df['Next_clinical_appointment'] >= df['Encounter_Date']]
    after = len(df)
    logging.info(f"Dropped {before - after} records due to invalid appointment order.")
    return df
...
def main():
    ...
    try:
        ...
        df = drop_invalid_appointment_pairs(df) 
